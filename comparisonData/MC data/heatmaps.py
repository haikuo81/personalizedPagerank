...
def draw_heatmap(df, title, xName, yName, zName, **args):
    ...
    ax.set_ylim(plt_y.min(), plt_y.max() + 1)
    cbar = fig.colorbar(cax, orientation="horizontal").set_label(zName)
    xTicks = [0,99,199,299,399,499,599,699,799]
    plt.xticks(xTicks)
...
def heatmaps(filename, title, undirected):
    ...
    for x in xList:
        for y in yList:
            for z in zList:
                if x != y:
                    xl = "random walks for each node"
                    yl = y
                    if x == "largeTop":
                        xl = "largeTop (L)"
                    if y == "max entries":
                        yl = "entries kept"
                    draw_heatmap(df, title, x, y, z, path = "", xlabel = xl, ylabel = yl)
